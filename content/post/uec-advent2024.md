...
葉を左から見たときに値の昇順になるため、$\mathrm{successor}(x)$や$\mathrm{predecessor}(x)$は$x$を表す葉のすぐに存在します。
この性質を利用して、次の工夫を行います。
...
二つ目についてはわかりにくいと思いますが、これはどちらかというと一つ目の連結リスト化を動的に行うために持つサポート用とも言えます。「すぐ近くになるはずの葉にジャンプ辺をはる」というのは、具体的には、
...
6は0110なので、左右と進みます。次に右に進むべきですが、辺がありません。そこで、ジャンプ辺を利用して左部分木最大の0101(5)にジャンプします。ここから連結リストで一つ右に移動し、1101(13)を手に入れることができます。図より、正しい解が返ってきていることがわかると思います。
...
### まとめ
数を固定長さで最上位桁からみる文字列として扱い、葉ノード間の連結リスト、ジャンプ辺の概念を導入することで、$\mathrm{insert}, \mathrm{remove}, \mathrm{successor}, \mathrm{predecessor}$を$O(w)$時間で行うことができる。
...
例として、下図において1100を探すことにします。
まず、根を高さ0として、高さ2のハッシュマップを見ます。
高さ2のハッシュマップに11が含まれているのでより下の高さを考えます。
...
### アイデア
二分探索木による解法、クエリ$O(\log(N))$がY-Fast Trieの性能に劣るのは、$w \ll N$であるからでした。
では、$N$が十分小さいことが保証できる状況であれば、二分探索木を利用することができます。
また、二分探索木はtrie系のデータ構造と異なり、一つの要素のために$O(w)$領域を必要としません。

そこで、本来trie側で扱うべきだったノードをいくつかのグループに分けて、それらを二分探索木に管理させることでこれらのメリットを全部享受することを狙います。
...
もし、挿入されるデータがあらかじめわかっていれば、$x _ i$としてどれを選べばよいかがわかりますが、実際の運用ではそうではありません。
...
### まとめ
Y-Fast TrieはX-Fast Trieのノードを間引き、扱う要素を乱数により小さい集合に分割する。こうして得た小さい集合を二分探索木に管理させることにより、$\mathrm{insert}, \mathrm{remove}, \mathrm{successor}, \mathrm{predecessor}$を$\text{expected } O(\log(w))$時間で行うことができる。
